...
// - 공통 인증 미들웨어(requireLogin) 사용으로 CORS 프리플라이트/토큰소스 일관화
// - 누적 카운터($inc) 반영 유지
// - 로깅/유효성 검증 유지
// ------------------------------------------------------------
...
const sharp = require('sharp'); // (현재 파일에선 미사용, 기존 유지)
const bcrypt = require('bcrypt'); // (현재 파일에선 미사용, 기존 유지)
...
/** ============================
 *  🤝 친구 신청 수락 (pending → accepted 원자적 전이)
 * ============================ */
...
router.put('/block/:id', requireLogin, async (req, res) => {
  try {
    ...
    log('🚫 일반 차단 완료', { myId, targetId, addedBlock: added, removedFriends, rejectedPending: modifiedCount });
