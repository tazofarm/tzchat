...
function startPollingComplete(identityVerificationId) {
  ...
  const loop = async () => {
    ...
    try {
      ...
      // ✅ 지금 네가 보고 있는 VERIFIED 응답이 여기에 찍힘
...
async function proceedRouteByTx(txId) {
  // ✅ 여기서부터가 “분기” 단계. 지금 너는 이 단계에서 멈추는 중일 가능성이 큼.
  try {
    ...
    // ✅ route 응답을 화면에 그대로 보여줌 (여기 보면 404/에러/성공이 바로 드러남)
    lastFailDetail.value = {
    ...
    // ✅ 여기 걸리면 “route가 아예 응답이 안 옴(서버 hang/프록시 문제)” 같은 케이스
...
async function finalizeByIdentityVerificationId(identityVerificationId) {
  try {
    ...
    // ✅ complete 성공을 화면에 남기고 route로 넘어감
    lastFailDetail.value = {
      stage: 'complete:ok',
    ...
    lastFailDetail.value = payload || { stage: 'complete:exception', message: e?.message || '', stackTop: String(e?.stack || '').split('\n')[0] }
    mode.value = 'fail'
    busy.value = false
    await closeExternal()
    clearRunningStore()
